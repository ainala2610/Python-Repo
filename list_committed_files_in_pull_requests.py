...
if response.status_code == 200:
    ...
    for number in pull_requests_list:
        ...
        url3 = f'https://api.github.com/repos/ainala2610/Python-Repo/pulls/{number}/files'
        response1 = requests.get(url2)
        response2 = requests.get(url3)
        data2 = response1.json()
        data3 = response2.json()
        pullrequest_owner = data2["user"]["login"]
        filename = data3[0]['filename']
        print(filename)
        print(f"The Pull Request {number} has been reaised by: {pullrequest_owner} and the committed files: {filename}")
